...
params = {'alpha': 0.001,
        ...
        'MODEL_XML_PATH': os.path.normpath(os.path.join(__file__, '../../src/dashgo/assets/reach.xml')),
        # 'env_name': 'xArm6Reach-v1',
        'policy_path': os.path.normpath(os.path.join(__file__, '../../src/models/model.pt')),
        'group_name': 'arm',
        'z_offset': 0.461}
...
def target_callback(msg) -> None:
    global scene, robot, trajectoryPlanner, arm, ready, client, path_strategy, params
    ready = False
    ...
    # Insert box at target position (account for 90 degree rotation because of dashgo change in frame of reference)
    p = PoseStamped()
    ...
    move_xarm6(trajectoryPlanner, goal, arm, client, path_strategy, z_offset)
...
if __name__ == '__main__':
    try:
        ...
        arm = moveit_commander.MoveGroupCommander(params['group_name'])
